...
export const IntegrationHub: React.FC<Props> = ({
  ...
  return (
    <section aria-labelledby="integrations-title" className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm space-y-4">
      ...
      {/* ── Google Assistant / IFTTT setup guide ─────────────────── */}
      <details className="group">
        <summary className="text-xs font-semibold text-gray-500 cursor-pointer hover:text-gray-700 list-none flex items-center gap-1">
          <svg className="w-3 h-3 transition-transform group-open:rotate-90" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
            <path fillRule="evenodd" d="M7.293 4.707a1 1 0 011.414 0l5 5a1 1 0 010 1.414l-5 5a1 1 0 01-1.414-1.414L11.586 10 7.293 5.707a1 1 0 010-1.414z" />
          </svg>
          Google Assistant (Gemini) — חיבור קולי
        </summary>

        <div className="mt-3 space-y-3">
          <p className="text-[11px] text-gray-500 leading-relaxed">
            אמור <em>&ldquo;Hey Google, הוסף משימה [שם]&rdquo;</em> והמשימה תגיע אוטומטית ללוח.
            החיבור עובד דרך IFTTT (גרסה חינמית) שמחבר בין האסיסטנט לאפליקציה.
            האפליקציה בודקת כל 30 שניות אם הגיעו פריטים חדשים.
          </p>

          <ol className="space-y-2 text-[11px] text-gray-600">
            <li className="flex gap-2">
              <span className="w-5 h-5 rounded-full bg-indigo-100 text-indigo-700 font-bold flex items-center justify-center shrink-0 mt-0.5">1</span>
              <span>
                ב-Vercel הוסף משתנה סביבה:<br />
                <code className="bg-gray-100 px-1.5 py-0.5 rounded font-mono text-[10px]">ASSISTANT_WEBHOOK_TOKEN=your_secret_token</code>
              </span>
            </li>
            <li className="flex gap-2">
              <span className="w-5 h-5 rounded-full bg-indigo-100 text-indigo-700 font-bold flex items-center justify-center shrink-0 mt-0.5">2</span>
              <span>
                ב-IFTTT צור Applet חדש:<br />
                <strong>If:</strong> Google Assistant → &ldquo;Say a phrase with a text ingredient&rdquo;<br />
                ניסוח: <em>הוסף משימה $</em> &nbsp;(הדולר = הטקסט שתגיד)
              </span>
            </li>
            <li className="flex gap-2">
              <span className="w-5 h-5 rounded-full bg-indigo-100 text-indigo-700 font-bold flex items-center justify-center shrink-0 mt-0.5">3</span>
              <span>
                <strong>Then:</strong> Webhooks → Make a web request<br />
                URL: <code className="bg-gray-100 px-1.5 py-0.5 rounded font-mono text-[10px] break-all">https://YOUR-APP.vercel.app/api/assistant-capture?token=your_secret_token</code><br />
                Method: <code className="bg-gray-100 px-1 rounded font-mono text-[10px]">POST</code>
                &nbsp;·&nbsp; Content-type: <code className="bg-gray-100 px-1 rounded font-mono text-[10px]">application/json</code><br />
                Body: <code className="bg-gray-100 px-1.5 py-0.5 rounded font-mono text-[10px]">{'{"text": "{{TextField}}", "source": "google-assistant"}'}</code>
              </span>
            </li>
          </ol>

          <p className="text-[11px] text-emerald-700 bg-emerald-50 border border-emerald-200 rounded px-3 py-2">
            לרשימת קניות: אמור &ldquo;הוסף לרשימת קניות [מוצר]&rdquo; — האפליקציה מזהה אוטומטית ומוסיפה לרשימה.
          </p>
        </div>
      </details>
