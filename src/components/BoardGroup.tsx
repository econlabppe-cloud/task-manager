...
const colorMap: Record<string, {
  ...
  header: string; darkHeader: string; darkText: string
}> = {
  purple: { bg: 'bg-purple-50', text: 'text-purple-900', border: 'border-purple-300', dot: 'bg-purple-500', header: 'bg-purple-100',  darkHeader: 'bg-purple-900/40', darkText: 'text-purple-200' },
  blue:   { bg: 'bg-blue-50',   text: 'text-blue-900',   border: 'border-blue-300',   dot: 'bg-blue-500',   header: 'bg-blue-100',    darkHeader: 'bg-blue-900/40',   darkText: 'text-blue-200'   },
  green:  { bg: 'bg-green-50',  text: 'text-green-900',  border: 'border-green-300',  dot: 'bg-green-500',  header: 'bg-green-100',   darkHeader: 'bg-green-900/40',  darkText: 'text-green-200'  },
  orange: { bg: 'bg-orange-50', text: 'text-orange-900', border: 'border-orange-300', dot: 'bg-orange-500', header: 'bg-orange-100',  darkHeader: 'bg-orange-900/40', darkText: 'text-orange-200' },
  teal:   { bg: 'bg-teal-50',   text: 'text-teal-900',   border: 'border-teal-300',   dot: 'bg-teal-500',   header: 'bg-teal-100',    darkHeader: 'bg-teal-900/40',   darkText: 'text-teal-200'   },
  red:    { bg: 'bg-red-50',    text: 'text-red-900',    border: 'border-red-300',    dot: 'bg-red-500',    header: 'bg-red-100',     darkHeader: 'bg-red-900/40',    darkText: 'text-red-200'    },
  pink:   { bg: 'bg-pink-50',   text: 'text-pink-900',   border: 'border-pink-300',   dot: 'bg-pink-500',   header: 'bg-pink-100',    darkHeader: 'bg-pink-900/40',   darkText: 'text-pink-200'   },
  indigo: { bg: 'bg-indigo-50', text: 'text-indigo-900', border: 'border-indigo-300', dot: 'bg-indigo-500', header: 'bg-indigo-100',  darkHeader: 'bg-indigo-900/40', darkText: 'text-indigo-200' },
}

const COLUMN_HEADERS = [
  { label: 'שם משימה', className: 'flex-1 min-w-0' },
  { label: 'אחראי',    className: 'w-12 text-center' },
  { label: 'סטטוס',    className: 'w-[116px] text-center' },
  { label: 'עדיפות',   className: 'w-20 text-center' },
  { label: 'יעד',      className: 'w-28 hidden sm:block' },
...
export const BoardGroup: React.FC<Props> = ({
  ...
  return (
    <article aria-label={`קבוצת משימות: ${group.title}`} className="mb-5" role="listitem">
      {/* ── Group Header ──────────────────────────────────────────── */}
      <div className={`flex items-center gap-3 px-4 py-3.5 rounded-t-xl ${headerBg} border-b ${darkMode ? 'border-white/10' : colors.border}`}>
        {/* Collapse */}
        <button
          onClick={() => onToggleCollapse(group.id)}
          aria-expanded={!group.collapsed}
          aria-controls={`group-body-${group.id}`}
          aria-label={group.collapsed ? `הרחב: ${group.title}` : `כווץ: ${group.title}`}
          className={`w-7 h-7 flex items-center justify-center rounded-lg hover:bg-black/10 transition-transform duration-200 touch-manipulation ${group.collapsed ? '-rotate-90' : ''}`}
        >
          <svg className={`w-4 h-4 ${headerText}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
          </svg>
        </button>

        {/* Color dot */}
        <div className={`w-3.5 h-3.5 rounded-full ${colors.dot} shrink-0 shadow-sm`} />
        ...
        {editingTitle ? (
          <input
            ...
            autoFocus
            dir="rtl"
            className={`text-base font-bold ${headerText} bg-white/20 border border-white/40 rounded-lg px-2 py-0.5 focus:outline-none min-w-0 flex-1 max-w-[220px]`}
          />
        ) : (
          <h3
        ...
        {confirmDelete ? (
          <div className="flex items-center gap-2 text-sm" role="alertdialog">
            <span className={`font-semibold ${darkMode ? 'text-red-400' : 'text-red-600'}`}>מחק?</span>
            <button onClick={() => onDeleteGroup(group.id)} className={`font-bold px-2 py-1 rounded ${darkMode ? 'text-red-400 hover:bg-red-900/40' : 'text-red-600 hover:bg-red-50'}`}>כן</button>
            <button onClick={() => setConfirmDelete(false)} className={`px-2 py-1 rounded ${darkMode ? 'text-gray-400 hover:bg-white/10' : 'text-gray-500 hover:bg-black/10'}`}>לא</button>
          </div>
        ) : (
          <button
            onClick={() => setConfirmDelete(true)}
            aria-label={`מחק קבוצה ${group.title}`}
            className={`${headerText} opacity-30 hover:opacity-70 transition-opacity touch-manipulation p-1`}
          >
      ...
      {/* ── Body ─────────────────────────────────────────────────── */}
      {!group.collapsed && (
        <div id={`group-body-${group.id}`} className={`border-x border-b rounded-b-xl ${bodyBg}`}>
          {/* Column headers (desktop only) */}
          <div className={`hidden sm:flex items-center px-1 py-2 border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          ...
          {filteredTasks.length === 0 ? (
            <div className={`px-4 py-5 text-sm text-center ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
          ...
          <AddTaskRow groupColor={group.color} darkMode={darkMode} onAdd={title => onAddTask(group.id, title)} />
        </div>
      )}
      ...
      {group.collapsed && (
        <button
          className={`w-full flex items-center gap-3 px-5 py-3.5 border border-t-0 rounded-b-xl cursor-pointer transition-colors touch-manipulation ${
            darkMode ? 'bg-gray-800 border-gray-700 hover:bg-gray-700' : 'bg-white border-gray-100 hover:bg-gray-50'
          }`}
          onClick={() => onToggleCollapse(group.id)}
          aria-label={`הרחב ${group.title}: ${filteredTasks.length} משימות, ${doneCount} הושלמו`}
        >
          <div className={`w-2.5 h-2.5 rounded-full ${colors.dot} shrink-0`} />
          <span className={`text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{group.title}</span>
